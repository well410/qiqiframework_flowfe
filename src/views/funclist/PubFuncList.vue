<template>
    <!--函数表格，所有产品下函数的信息-->
    <el-card class="box-card" :body-style="{padding: '0'}" id="pubFuncCard">
        <!--操作工具栏-->
...
<!--                                    </div>-->
            ...
            <div v-else class="no-data">
                <svg-icon icon-class="star_pubfunc"/>
                <br/>
                <br/>
                <span>未查询到公有函数数据</span>
            </div>
        </div>
        <el-pagination
        ...
        <!--历史界面-->
        <el-dialog width="80%" heigh="95%" :visible.sync="his.dialog" :close-on-click-modal="false"
                   class="text-font-family">
            <div slot="title" width="80%" heigh="95%">
                <span>{{his.title}}</span>
                <i class="el-icon-question icon-Add-style" @click="tohelperHis"></i>
            </div>
            <function-history
                    ref="hisFrom"
                    :dialogTitle="his.title"
                    :dialogFormVisible="his.dialog"
                    :func="his.func"
                    :prod="his.prod"
                    v-on:visibleChange="onVisibleChange($event)">
            </function-history>
        ...
        <el-dialog width="65%" heigh="95%" :visible.sync="debug.dialog" :close-on-click-modal="false"
            ...
            <pub-debug-function ref="debugFrom"
                            :dialogTitle="debug.title"
                            :dialogFormVisible="debug.dialog"
                            :func="debug.func"
                            :prod="debug.prod"
                            v-on:visibleChange="onVisibleChangeDebug($event)"></pub-debug-function>
        </el-dialog>
        <!--版本界面-->
...
<script>
    // 页面组件
    import ShowFunction from '../../components/pub-function/ShowFunction'
    import AddFunction from '../../components/pub-function/AddFunction'
    import UpdateFunction from '../../components/pub-function/UpdateFunction'
    import FunctionVersion from '../../components/pub-function/FunctionVersion'
    import FunctionHistory from '../../components/pub-function/FunctionHistory'

    // entity,dto,常量
    import {FuncFilter, FuzzyFuncFilter} from "../../api";
    import {funcStatusEnums} from "../../components/pub-function/index";
    ...
    import PubDebugFunction from "../../components/debug-comp/PubDebugFunction";
        ...
        methods: {
            ...
            funcDelectClick(func) {
                ...
                this.$confirm('确定要删除该函数吗？', '提示', {
                    ...
                    deleteFunc({prodId: "pub", funcId: func.funcId}).then(res => {
            ...
            //     that.his.dialog = true;
            //     that.$nextTick(()=>{
            //         that.$refs['hisFrom'].init();
            //     });
            // },
            funcHisClick(func) {
                let that = this;
                that.his.func = new FuncBase(func);
                this.$router.push({
                    path: "/pubFuncHome/pubFuncHistory",
            ...
            funcVersionClick(func) {
                ...
                this.$router.push({
                    path: "/pubFuncHome/pubFuncVersion",
            ...
            funcCitationClick(func){
                ...
                debugger
                this.$router.push({
                    path: "/pubFuncHome/pubFuncCitation",
            ...
            funcCheckClick(func) {
                ...
                if (func.status === '0') {
                    // 调用验证接口，后端验证成功后就更改状态
                    checkFunc({prodId: "pub", funcId: func.funcId}).then(res => {
            ...
            funcPublishClick(index, func) {
                ...
                this.$confirm('确定要发布版本吗？', '提示', {
                    ...
                    if (func.status === '1') {
                        // 调用发布接口
                        publishFunc({prodId: "pub", funcId: func.funcId}).then(res => {
            ...
            multiPublicClick() {
                ...
                if (checkPublishSelectedFuncVersionList(selectData, that)) {
                    ...
                    multiPublishFunc({prodId: "pub", funcBaseList: selectData}).then((res) => {
