<template>
    <div id="modelInterface">
                ...
                <el-form-item>
                    ...
                    <div class="tools-bar" style="margin-bottom: 8px;width: 200px">
                        ...
                        <div class="tools-button">
                            <el-button type="primary" size="mini" icon="el-icon-plus" @click="addClick" v-cmb-auth-remove="'FrontModelCmptAddCode'">
                                新增
                            </el-button>
                            <el-button type="success" size="mini" @click="publishClick" v-cmb-auth-remove="'FrontModelCmptPublishCode'">
        ...
        <div v-loading="loading" element-loading-background="#fff0">
            <div v-if="modelInterfaceList.length != 0"  v-show="modelInterfaceList.length != 0" v-loading="loading">
                <el-checkbox-group v-model="checkList">
                    <div style="display: flex;flex-wrap: wrap; justify-content: space-between;min-height: 719px" >

                        <div  class="card-block" v-for="(o,index) in modelInterfaceList" >
                            <el-checkbox :label="o">
                                <el-card :body-style="{ padding: '0px' }">
                                    ...
                                    <div class="bottom">
                                        <div>
                                            <el-button class="icon-button" type="primary" size="mini" icon="el-icon-edit" @click="editClick(o)" v-cmb-auth-remove="'FrontModelCmptEditCode'" title="编辑" style="padding: 7px 30px;"></el-button>
                                            <el-button class="icon-button" type="primary" size="mini" icon="el-icon-video-play" @click="debugClick(o)" v-cmb-auth-remove="'FrontModelCmptEditCode'"title="调试" style="padding: 7px 30px;"></el-button>
                                            <el-button class="icon-button" type="primary" size="mini" icon="el-icon-check" @click="verifyClick(o)" v-cmb-auth-remove="'FrontModelCmptCheckCode'" title="验证" style="padding: 7px 30px;"></el-button>
                                        </div>
                                        <div>
                                            <el-button class="icon-button" type="primary" size="mini" icon="el-icon-files" @click="versionClick(o)" v-cmb-auth-remove="'FrontModelmptVersionCode'" title="版本" style="padding: 7px 30px;"></el-button>
                                            <el-button class="icon-button" type="primary" size="mini" icon="el-icon-document" @click="hisClick(o)" v-cmb-auth-remove="'FrontModelCmptHistoryCode'"title="历史" style="padding: 7px 30px;"></el-button>
                                            <el-button class="icon-button" type="danger" size="mini" icon="el-icon-delete" @click="deleteClick(o)" v-cmb-auth-remove="'FrontModelCmptDelectCode'" title="删除" style="padding: 7px 30px;"></el-button>
                                        </div>
                                    </div>
                                </el-card>
                            </el-checkbox>
            ...
            <div v-else class="no-data">
                <svg-icon icon-class="list_model"/>
                <br/>
                <br/>
                <span>未查询到模型组件数据</span>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            tohelperHome(){
                window.open(this.helper.helpUri.uri+'/modules/component/4.html#模型组件配置',"_blank")
            },
            toggle(showStatus){
                return !showStatus
            },
            ...
            refreshPage () {
                ...
                queryFuzzyCmptBasePagedAndCiteCount(this.prod.prodId,this.cmptQueryDto).then(res=>{
                    if (res.data!=null ){
                        ...
                        that.modelInterfaceList=res.data.data["objectList"];;
                        that.showCiteCount(that.modelInterfaceList);
                    }
                }).catch((res) => {
                }).finally(() => {
                    setTimeout(() => {
            ...
            showCiteCount(modelInterfaceList){
                let that=this;
                let modelList=modelInterfaceList.map(item =>{
                    return {prodId:item.prodId,cmptId:item.cmptId};
                });
                this.decsCounts=[];
                queryCiteCount(this.prod.prodId,modelList).then(res=>{
            ...
            addClick () {
                this.$router.push({path: this.routePath.modelActionPath, query: {act: this.actType.modelAdd, title: "模型新增"}});
            },
            /**
             * 删除接口
             * */
            ...
            editClick (row) {
                this.$router.push({
                    ...
                    query: {cmptId: row['cmptId'], act: this.actType.modelEdit, title: '模型组件'}
                });
            },
            /**
             * 调试
            ...
            debugClick (row) {
                this.$router.push({
                    path: this.routePath.modelActionPath,
                    query: {cmptId: row['cmptId'], act: this.actType.modelDebug, title: '模型组件调试'}
                });
            },
            /**
             * 验证
            ...
            verifyClick (row) {
                ...
                let that = this;
                saveCmptBase(this.prod.prodId, 'modelBase', rowTmp).then(() => {
                    successTip('验证组件成功');
                    that.refreshPage();
                }).catch(() => {
                    // errorTip('验证组件失败')
            ...
            hisClick (row) {
                this.$router.push({
                    path: this.routePath.modelHistoryPath,
            ...
            versionClick (row) {
                this.$router.push({
                    path: this.routePath.modelVersionPath,
                    query: {cmptId: row['cmptId']}
                });
            },
            cmptCitationClick(cmpt){
                this.$router.push({
                    path: this.routePath.modelCitationPath,
