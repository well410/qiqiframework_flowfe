...
export const isContainEmpty = (data) => {
  return data.filter(item => isEmpty(item)).length > 0
};

export const isNotContainEmpty = (data) => {
  return data.filter(item => isEmpty(item)).length === 0
};
 ...
 * @param data
 */
export const toCamelName = (name) =>{
  let lowerCaseName = name.toLowerCase();
  return lowerCaseName.replace(/\_(\w)/g, function(all, letter){
    return letter.toUpperCase();
  });
}